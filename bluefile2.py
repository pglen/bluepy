...
def mainfunct():
    ...
    if conf.passwd == "":
        ...
        if conf.encrypt:
            ...
            print( \
            "Prompting for pass. Make sure you enter correct pass, as the data\n"\
            "decoded will not match the original.")

        conf.passwd = getpass.getpass("Enter password for file: ")
        #print(  conf.passwd)
        #sys.exit(2)
        if not conf.passwd:
            print("Must enter pass.")
            sys.exit(3)
    ...
    while 1:
        ...
        bytes += len(buff)
        if conf.encrypt:
            buff2 = bluepy2.encrypt(buff, conf.passwd)
        if conf.decrypt:
            buff2 = bluepy2.decrypt(buff, conf.passwd)
